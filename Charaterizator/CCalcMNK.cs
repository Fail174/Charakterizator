...
namespace Charaterizator
{
    class CCalcMNK
    {
        ...
        // Коэффициент регулировки при расчете шага веса 
        public double KdM;
        // Допускаемая минимальная граница погрешности ниже которой нет смысла считать
        public double deltaFdop_min;
        // Минимальная расчетная граница
        //public double Fr_min;
        public int Res_count_max; 
        //-----------------------------------------------------------------------------------
        ...
        public Matrix<double> CalcCalibrCoef(Matrix<double> Rmtx, Matrix<double> Umtx, Matrix<double> Pmtx, Matrix<double> Tmtx, double Pmax, Matrix<double> gammaP, Matrix<double> gammaT, bool sensor_DV)
        {
            ...
            int Res_count = 0;          // Счетчик количества всех найденных решений
        ...
        int rowP = Pmtx.RowCount;           // Количество строк матрицы Pmtx
            ...
            for (int N = 0; N < rowP; N++)
            {
                ...
                {
                    sumFkn = sumFkn + Math.Abs(Fkn1.At(N, K));
                }
            }
            sumFkn = sumFkn / (rowP*colP);

            if (sumFkn > deltaFdop_min)

            {
                ...
                for (int Kf = 1; Kf < Kpmax_dop; Kf += 2)  // Изменил предельное значение "10" на "Kpmax_dop"
                {
                ...
                while (Kmult >= 0.1)
                {
                    ...
                    while (Exit_dw == false)
                    {
                        ...
                        if (Ndop == Nmax)
                        {
                            ...
                            Fr = CalcFr_cur(rowP, colP, Fr, Fkn, deltaFdop_min);
                        ...
                        if ((maxValueM > Mmax) || (Tdw_out_res > Tdw_out_res_max)||(Res_count > Res_count_max))
                        {
            ...
            else // для if (sumFkn > deltaFdop_min)
            {
                M_opt = M1; // идеальное решение получено с единичной матрицей М
        ...
        */
        ...
        public Matrix<double> CalcFr_cur(int rowP, int colP, Matrix<double> Fr, Matrix<double> Fkn, double deltaFdop_min)
        {
            double deltaFkn_min = 100;
            ...
            for (int N = 0; N < rowP; N++)
            {
                for (int K = 0; K < colP; K++)
                {
                    ...
                    if (Fr_cur.At(N, K) < deltaFdop_min)
                    {
                        Fr_cur[N, K] = deltaFdop_min;
                    }
                }
            }
